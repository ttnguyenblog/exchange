<#
    ...
    V1.20, 07/08/2024 - Fixed errors when using remote PowerShell
#>
    ...
    2 {
        ...
        Foreach ($Mbx in $AllMailbox) {
            ...
            if ($Mbx.ArchiveName -ge "1") {
                $MbxArchiveStats = Get-mailboxstatistics $Mbx.distinguishedname -Archive -ErrorAction SilentlyContinue
            ...
            if ($Stats) {
                [string]$TempSize = $Stats.TotalItemSize.Value
                [long]$TotalSize = $TempSize.Substring($TempSize.IndexOf("(") + 1).Replace(" bytes)", "").Replace(",", "")
                $userObj | Add-Member NoteProperty -Name "TotalItemSize" -Value ([math]::round($TotalSize / 1MB))
                $userObj | Add-Member NoteProperty -Name "ItemCount" -Value $Stats.ItemCount
                $userObj | Add-Member NoteProperty -Name "DeletedItemCount" -Value $Stats.DeletedItemCount
                [string]$TempDelSize = $Stats.TotalDeletedItemSize.Value
                [long]$TotalDelSize = $TempDelSize.Substring($TempDelSize.IndexOf("(") + 1).Replace(" bytes)", "").Replace(",", "")
                $userObj | Add-Member NoteProperty -Name "TotalDeletedItemSize" -Value ([math]::round($TotalDelSize / 1MB))
            }
